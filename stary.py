...
def main(argv):
    ...
    # Edge Cascade
    canny = cv.Canny(bilateral, 115, 175)
 ...
 # tarcza strzelecka
    ...
    circles = cv.HoughCircles(gray, cv.HOUGH_GRADIENT, 1, 0.01,
...
def countHitScore(hits, delta_r, target_center):
    ...
    for hit in hits[0]:
        ...
        score += 11 - math.ceil(total_dist / delta_r)
        # print(f'{total_dist / delta_r} math')
        print(f'{total_dist / delta_r} total_dist / delta_r')
        print(f'{11 - math.ceil(total_dist / delta_r)} rownanie')
